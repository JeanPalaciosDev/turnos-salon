...
>
> **Build EAS actual:** tras tres rechazos `403` de upload, una subida posterior fue aceptada y creó el build Android `a6616e7c-1f7f-4bca-957a-0db0bf31fdf5` con perfil `development`. Su último estado verificado es `IN_PROGRESS`; aún no hay artefacto descargable. Esta nota prevalece sobre las menciones históricas a los rechazos iniciales en el resto del documento.

## 1. Estado real de implementación

| Área | Estado | Evidencia / alcance actual |
|---|---|---|
| Monorepo | Parcialmente implementado | Turborepo, `@turnos/core`, `@turnos/models`, Expo y Supabase están creados. Core y models tienen build TypeScript; el build móvil y el de Supabase siguen requiriendo EAS/CLI para producir artefactos reales. |
...
| Calidad y release | Parcialmente preparado | TypeScript, configuración Expo, perfil EAS de desarrollo, assets y migraciones locales fueron verificados. El proyecto EAS y sus variables de development están vinculados, pero dos subidas del archivo de build fallaron con `403` antes de crear un build remoto. No hay tests, E2E ni binario nativo validado. |
...
| Distribución | EAS Build + `expo-dev-client` 5.0.20 | `apps/mobile/eas.json` define perfiles; proyecto y variables development están vinculados. El upload remoto aún es rechazado con `403`. |
...
apps/
  mobile/
    eas.json                 perfiles EAS: development, preview y production
    app/
...
### Configuración por entorno y builds

- Copiar `apps/mobile/.env.example` a `apps/mobile/.env.local` y cargar la URL/anon o publishable key de Supabase.
...
- `expo-dev-client` `5.0.20` y `apps/mobile/eas.json` habilitan el perfil `development` con distribución interna. El proyecto `@jeancydev/turnos-salon` está vinculado en `app.json`; las variables públicas de Supabase están guardadas en el entorno EAS `development` como `Sensitive`.
- Dos intentos de EAS resolvieron ese entorno y las credenciales Android, pero fallaron con `403` durante la subida de metadata y tarball. No existe aún un build remoto ni un APK descargable.
...
|---|---|---|---|
| 1 — Fundaciones | Monorepo, core/models, Supabase, WatermelonDB y sync | Código, validación local de migraciones/RLS, perfil EAS y proyecto cloud vinculados; la subida a EAS está bloqueada por `403` | Build de paquetes, sync probado en development build y contra PostgreSQL local/hosted |
...
1. **Task 1 — Scaffolding:** estructura, assets, `expo-dev-client`, perfiles EAS y proyecto cloud completados. Pendiente resolver el `403` de upload y verificar un build nativo; Tamagui no forma parte de la implementación actual.
...
6. **Task 6 — ABM de trabajadores:** siguiente módulo funcional después de la validación E2E. Requiere primero un flujo seguro de invitación/provisión de `auth.users` y vinculación de `worker_id`; la base de privacidad RLS/sync ya está preparada.
...
- Resolver el `403` de EAS durante la subida y generar el perfil `development` antes de probar sign-up, login, recuperación de sesión, bootstrap, sign-out y cambio de cuenta en un development build.
...
- Resolver o reintentar `expo-doctor` hasta completar sus 18 chequeos: la ejecución actual completó 16/18; el chequeo remoto del schema recibió HTML en lugar de JSON y React Native Directory marca WatermelonDB como no probado en New Architecture. Revisar también las vulnerabilidades transitivas y validar un binario EAS antes de una release.
...
- `app.json` contiene ahora el owner y `projectId` de EAS. No se debe vincular otra cuenta o proyecto sin confirmación, ya que los builds y las credenciales Android quedarían asociados a un destino distinto.
- `expo-doctor` informa que `@nozbe/watermelondb` no está probado por React Native Directory para New Architecture. Esto no demuestra incompatibilidad, pero requiere validar SQLite/JSI en el development build antes de una release o de cambiar la arquitectura nativa.
- La auditoría de dependencias de producción (`npm audit --omit=dev`) registra 31 vulnerabilidades (18 moderadas, 12 altas y 1 crítica). La corrección de `tar` y de varias cadenas Expo exige Expo `57.0.16`; los hallazgos de Metro exigen React Native `0.86.3`. Son migraciones mayores respecto a Expo SDK 52/RN 0.76.9, por lo que no se debe ejecutar `npm audit fix` ni aplicar overrides sin un plan de actualización y validación nativa aislados.
- EAS resolvió correctamente las variables `development` y las credenciales Android, pero rechazó dos uploads con `403` antes de crear un build. El diagnóstico local confirmó conectividad HTTPS a Expo y ausencia de proxy/PAC WinHTTP; los focos restantes son la autorización o verificación de la cuenta/proyecto EAS y una VPN, DNS o red que afecte específicamente la solicitud de subida firmada. No se debe seguir reintentando sin aislar ese acceso.
